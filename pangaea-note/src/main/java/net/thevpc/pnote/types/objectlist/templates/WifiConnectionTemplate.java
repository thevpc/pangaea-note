...
public class WifiConnectionTemplate extends AbstractPangaeaNoteTemplate {
    ...
    public void prepare(PangaeaNote n, PangaeaNoteService service) {
        ...
        PangageaNoteObjectDocument doc = new PangageaNoteObjectDocument().setDescriptor(new PangaeaNoteObjectDescriptor()
                ...
                .addField(new PangaeaNoteFieldDescriptor().setName(str("security", service)).setType(PangaeaNoteFieldType.COMBOBOX)
                        .addValue(str("security.wpa", service))
                        .addValue(str("security.wep", service))
                        .addValue(str("security.none", service))
                )
                .addField(new PangaeaNoteFieldDescriptor().setName(str("ssid", service)).setType(PangaeaNoteFieldType.TEXT))
                .addField(new PangaeaNoteFieldDescriptor().setName(str("passphrase", service)).setType(PangaeaNoteFieldType.PASSWORD))
        ...
        n.setContentType(PangaeaObjectListService.OBJECT_LIST);
        n.setContent(s.stringifyDescriptor(doc.setValues(new ArrayList<>(Arrays.asList(doc.getDescriptor().createObject())))));
    }
